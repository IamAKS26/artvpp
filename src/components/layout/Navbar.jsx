    ...
    return (
        <>
            <nav
                ...
                ${isScrolled ? 'bg-brand-light-bg/95 dark:bg-brand-dark-bg/95 backdrop-blur-xl border-b border-on-light-border/20 dark:border-on-dark-border py-2 shadow-sm' : 'bg-white/80 dark:bg-black/50 backdrop-blur-md md:bg-transparent py-4'}`}
            >
                    ...
                    <div className="flex items-center justify-between h-16">
                        ...
                        <div className="md:hidden flex items-center gap-4">
                            ...
                            <button
                                ...
                                className="p-2 -mr-2 text-on-light-text dark:text-white rounded-md bg-white/20 dark:bg-black/20 backdrop-blur-sm"
                            >
                                {isMenuOpen ? <X /> : <Menu />}
                            </button>
                        </div>
            ...
            {/* Mobile Menu (Full Screen Overlay) - MOVED OUTSIDE NAV to avoid transform issues */}
            <AnimatePresence>
                {isMenuOpen && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        className="fixed inset-0 z-40 bg-white/95 dark:bg-black/95 backdrop-blur-xl md:hidden pt-24 px-6 flex flex-col"
                    >
                        <div className="flex flex-col space-y-6 text-center flex-1 justify-center">
                            {navLinks.map((link) => (
                                <NavLink
                                    ...
                                    className="text-4xl font-display font-bold text-on-light-text dark:text-white hover:text-brand-primary transition-colors"
                                >
                                    {link.name}
                                </NavLink>
                            ))}
                        ...
                        <div className="pb-12 space-y-4 max-w-xs mx-auto w-full">
                            {isAuthenticated ? (
